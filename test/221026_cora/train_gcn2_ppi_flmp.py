...
from gnn_neo.quantization.utils import get_mp_alphas, get_mp_params_cost, get_mp_weights
...
model = FastLayerWiseMultiPrecisionQuantizer()(model)
optimizer_weight = torch.optim.Adam(get_mp_weights(model), lr=0.001)
optimizer_alpha = torch.optim.Adam(get_mp_alphas(model).values(), lr=0.001)
...
def train():
    ...
    for data in train_loader:
        ...
        loss_acc = criterion(model(data.x, data.adj_t), data.y)
        loss_mp = get_mp_params_cost(model)
        loss = loss_acc + 0.03*loss_mp*(loss_acc.detach().clone()/loss_mp.detach().clone())
        loss.backward()
