...
import os
os.makedirs(os.path.join(osp.dirname(osp.realpath(__file__)), '..', 'run', 'qm9_nn_conv_lmp', dt))
...
for epoch in range(1, 301):
    if epoch == 1:
        enable_calibration(model, True)
    elif epoch == 5:
        enable_calibration(model, False)
        enable_quantization(model, True)
    ...
    torch.save(model.state_dict(), os.path.join(osp.dirname(osp.realpath(__file__)), '..', 'run', 'qm9_nn_conv_lmp', dt, '%d'%(epoch,)))
