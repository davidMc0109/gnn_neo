import torch.nn
from gnn_neo.quantization.quantizer.mapping.nn_to_flmp import linear_to_flmp, conv1d_to_flmp, conv2d_to_flmp, conv3d_to_flmp
...
def _get_default_mapping():
    mapping = {}
    mapping[torch.nn.Linear] = linear_to_flmp
    mapping[torch.nn.Conv1d] = conv1d_to_flmp
    mapping[torch.nn.Conv2d] = conv2d_to_flmp
    mapping[torch.nn.Conv3d] = conv3d_to_flmp
    return mapping
...
class FastLayerWiseMultiPrecisionQuantizer(QuantizerBase):
    def __init__(self, qconfig=None):
        super(FastLayerWiseMultiPrecisionQuantizer, self).__init__(qconfig=qconfig)
